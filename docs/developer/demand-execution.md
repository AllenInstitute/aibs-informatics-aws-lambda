...
| `cdk` | `aibs-informatics-cdk-lib` | Infrastructure and orchestration: Step Functions topology, EFS, Batch, and the reference app. |
...
> **On `cdk-lib` line numbers.** The `cdk-lib` working tree in this workspace sits on
> `feature/ecr-mirroring`, whose changes to `demand_execution.py`, `data_sync.py`, and `efs.py` are
> confined to how a Docker image URI is resolved (`resolve_image_uri` / `DockerAssetLike`) and do
> not alter topology — but they **do** shift line numbers by one to a few lines. All `cdk:` citations
> here are against `main`, verified by direct read.
